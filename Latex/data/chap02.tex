...
\begin{figure}[htb]
  ...
  \subcaptionbox{背景被前景遮挡\label{fig:sub:occ1}} %标题的长度，超过则会换行，如下一个小图。
    {\includegraphics[width=0.4\textwidth]{occ1.png}}%
 \hspace{1em}%
  \subcaptionbox{其它遮挡情况\label{fig:sub:occ2}}
      {\includegraphics[width=0.4\textwidth]{occ2.png}}
...
\begin{figure}[htb]
  ...
  \subcaptionbox{输入图像\label{fig:sub:hole1}} %标题的长度，超过则会换行，如下一个小图。
    {\includegraphics[width=0.4\textwidth]{hole1.jpg}}%
 \hspace{1em}%
  \subcaptionbox{第一阶段区域合并结果\label{fig:sub:hole2}}
      {\includegraphics[width=0.4\textwidth]{hole2.jpg}}
...
\begin{figure}[htb]
  \centering%
      {\includegraphics[width=0.9\textwidth]{sgrm.png}}\\
...
\begin{figure}[htb]
  ...
  \subcaptionbox{输入图像\label{fig:sub:fig}} %标题的长度，超过则会换行，如下一个小图。
    {\includegraphics[width=0.25\textwidth]{fig.png}}%
 \hspace{1em}%
  \subcaptionbox{区域合并结果\label{fig:sub:seg}}
      {\includegraphics[width=0.25\textwidth]{seg.png}}
...
\begin{figure}[htb]
  \centering%
      {\includegraphics[width=0.9\textwidth]{segcom.png}}\\
...
\section{本章小结}
图像显著性区域检测是当前计算机视觉相关领域备受关注的研究热点区域之一。准确提取图像中显著性对象是识别和理解图像的重要第一步。虽然人类视觉系统与生俱来具备这种能力，但是在计算机上实现达到甚至超越人类视觉系统的识别准确度的显著性检测算法却较为困难。虽然近些年提出的一些算法大大提高了算法的效率和准确度~\cite{ChengPAMI,ufo,Yan2014Hierarchical,geodesicDistance}，对于一些困难的情况，例如背景复杂度高、前景与背景相近等，目前已有的算法仍无法得到正确结果。\par
本章对图像显著性检测问题进行了研究。以将图像分为显著性对象和背景2个区域为目标,提出了一种显著性引导的区域合并算法。由于在区域合并的不同阶段采取了不同的合并策略，本章算法能有效的防止在合并过程中将前景显著性对象错误的合并到背景区域。为了得到准确的显著性对象，本章算法利用背景线索进行区域显著性分析，不断将非显著性区域合并到背景区域。最后，将合并过程中得到的非背景区域作为候选显著性区域，通过加权平均的方式得到最终结果。区别于当前主流算法的主要思路，本章算法采用了基于背景的线索和图像合并的方式提取显著性对象。与前景线索相比，背景线索的通用性更强。因为前景\par
在2个公开数据集上测试了本文所提算法,实验结果证明了本文所提算法的有效性.在主要是自然图像构成,难度较大的ECSSD数据集上的实验中,本文算法的准确度和误差指标均优于其它算法.本文算法简单直观,且效率高,易于实现.针对背景连续性不足的图像,本文算法无法有效的通过区域合并将背景部分合并在一起,同时可能会将前景错误的合并到背景中. 为解决这一问题,在下一步的工作中将考虑引入色彩相似性之外的其它度量方式以及多尺度分析方法来更好的评估图像背景区域之间的相似性.
